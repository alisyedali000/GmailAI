//
//  AddStroke.swift
//  AIReply
//
//  Created by Syed Ahmad  on 01/03/2026.
//


import Foundation
import SwiftUI
import UIKit

extension View {

    func addStroke(radius: CGFloat, color : Color, lineWidth: CGFloat) -> some View {
        modifier(AddStroke(radius: radius, color: color, lineWidth: lineWidth))
    }

    func addBackground(color: Color) -> some View {
        modifier(Background(color: color))
    }

    /// Binds the view to the ViewModel’s error state and shows an alert when `showError` is true. Dismissing the alert clears error state via `dismissAlert()`.
    func alert(for viewModel: ViewModel) -> some View {
        alert("Error", isPresented: Binding(
            get: { viewModel.showError },
            set: { if !$0 { viewModel.dismissAlert() } }
        )) {
            Button("OK") { viewModel.dismissAlert() }
        } message: {
            Text(viewModel.errorMessage)
        }
    }

    /// Full-screen loader overlay when `visible` is true. Use with ViewModel’s `showLoader`.
    func loaderOverlay(visible: Bool, message: String = "Loading…") -> some View {
        overlay {
            if visible {
                ProgressView(message)
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                    .background(Color(UIColor.systemBackground).opacity(0.8))
            }
        }
        .allowsHitTesting(!visible)
    }
}

struct AddStroke : ViewModifier {
    
    var radius: CGFloat
    var color: Color
    var lineWidth: CGFloat
    
    func body(content: Content) -> some View {
        content
            .overlay {
                RoundedRectangle(cornerRadius: radius)
                    .stroke(color, lineWidth: lineWidth)
            }
    }
}
extension Color {
    
    init(hex: String) {
        let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int: UInt64 = 0
        Scanner(string: hex).scanHexInt64(&int)
        let a, r, g, b: UInt64
        switch hex.count {
        case 3: // RGB (12-bit)
            (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
        case 6: // RGB (24-bit)
            (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
        case 8: // ARGB (32-bit)
            (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
        default:
            (a, r, g, b) = (1, 1, 1, 0)
        }

        self.init(
            .sRGB,
            red: Double(r) / 255,
            green: Double(g) / 255,
            blue: Double(b) / 255,
            opacity: Double(a) / 255
        )
    }
}

struct Background: ViewModifier {
    let color: Color
    func body(content: Content) -> some View {
        
        ZStack {
            Rectangle()
                .foregroundStyle(color)
                .ignoresSafeArea(.all)
            
            content
        }
    }
}
